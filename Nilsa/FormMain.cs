...
namespace Nilsa
{
	public partial class FormMain : Form
	{
		...
		*/
		...
		//    FormEditPersonenDB formEditPersoneDb = new FormEditPersonenDB(this);
		//    formEditPersoneDb.sContHar = new String[iPersHarCount, iPersHarAttrCount + 1];
		//    for (int i = 0; i < iPersHarCount; i++)
		//    {
		//        for (int j = 0; j < iPersHarAttrCount; j++)
		//            formEditPersoneDb.sContHar[i, j] = sPersHar[i, j];
		//        formEditPersoneDb.sContHar[i, iPersHarAttrCount] = "";
		//    }
		//    formEditPersoneDb.iContHarCount = iPersHarCount;
		//    formEditPersoneDb.iContHarAttrCount = iPersHarAttrCount;
		//    formEditPersoneDb.Setup("Autorization Error select user", -1, true, userLogin, userPassword);

		//    if (lstPersoneChange.Count > 0)
		//    {
		//        clearTrashList(lstPersoneChange);
		//        foreach (String str in lstPersoneChange)
		//            for (int i = 0; i < formEditPersoneDb.lvList.Items.Count; i++)
		//            {
		//                if (formEditPersoneDb.lvList.Items[i].SubItems[1].Text == str)
		//                {
		//                    formEditPersoneDb.lvList.Items[i].Checked = true;
		//                    break;
		//                }
		//            }
		//    }

		//    formEditPersoneDb.ShowDialog();
		...
		//        if (formEditPersoneDb.suSelSocialNetwork != SocialNetwork)
		//        {
		//            SocialNetwork = formEditPersoneDb.suSelSocialNetwork;
		//            OnSocialNetworkChanged();
		//        }
		//        onAfterPersonenListChanged();

		//        userLogin = formEditPersoneDb.suSelLogin;
		...
		*/
		...
		public async void Setup(String sUsrSelLogin = "", String sUsrSelPwd = "", String sUsrSelID="")
		{
			...
			//{
			//    WaitNSeconds(1);
			//} while (!tbStartServiceIsClickedNow);
		...
		private int ContactsList_GetUserIdx(String sUD, List<string> listConts)
		{
			int iuserIdx = -1;
			for (int i = 0; i < listConts.Count; i++)
			{
				String str = listConts[i];
		...
		/// <returns></returns>
		private long GetContactIdByParametrValue(int parametrId, string parametrValue, long persId)
		{
			long contId = -1;
  ...
  //      private async Task SelectNextReceivedMessageAsync(bool bDelete = true, bool bBoycottCurrent = false)
		...
		private void SetPersoneParametersValues()
		{
			...
			if(photoPersURL != "" )
			{
				if (File.Exists(photoPersURL))
				{
                    try
                    {
                        using (var image = Image.FromFile(photoPersURL))
                        {
                            buttonEditPersHarValues.BackgroundImage = (Image)image.Clone();
                        }
                    }
                    catch (Exception e)
                    {
						...
						if (photoPersURL != "")
						{
							if (File.Exists(photoPersURL))
							{
                                try
                                {
                                    using (var image = Image.FromFile(photoPersURL))
                                    {
                                        button.BackgroundImage = (Image)image.Clone();
                                    }
                                }
                                catch (Exception e)
                                {
					...
					else
					{
						if (photoContURL != "")
						{
							if (File.Exists(photoContURL))
							{
                                try
                                {
                                    using (var image = Image.FromFile(photoContURL))
                                    {
                                        button.BackgroundImage = (Image)image.Clone();
                                    }
                                }
	  ...
	  //                  toolTipMessage.SetToolTip(labelCont1FIO, labelCont1FIO.Text);
		...
		private void LoadPersoneParametersValues()
		{
			...
			if (File.Exists(Path.Combine(sDataPath, "persone_name_" + getSocialNetworkPrefix() + Convert.ToString(lPersID) + ".txt")))
			{
				try
				{
					dbUserName = File.ReadAllText(Path.Combine(sDataPath, "persone_name_" + getSocialNetworkPrefix() + Convert.ToString(iPersUserID) + ".txt"));
		...
		private bool Autorize(String sUsrSelLogin = "", String sUsrSelPwd = "", String sUsrSelID = "")
		{
			...
			do
			{
				if (bShowAutorizeForm)
				{
					...
					formEditPersoneDb.iContHarCount = iPersHarCount;
					formEditPersoneDb.iContHarAttrCount = iPersHarAttrCount;
					formEditPersoneDb.Setup("Autorization Error select user", -1, false, userLogin, userPassword);
		...
		private bool notAutorize(String sUsrSelLogin = "", String sUsrSelPwd = "", String sUsrSelID = "")
		{
			Boolean bShowAutorizeForm = sUsrSelLogin.Length == 0 || sUsrSelPwd.Length == 0;
				...
				if (!bShowAutorizeForm)
				{
					...
					if (SocialNetwork == 0)
					{
						try
						{
							...
							bool bAutorizeAccept = false;
							if (curuid != -1) bAutorizeAccept = true;// && curuid == fwbVKontakte.loggedPersoneID)
		...
		public String TinderGetUserRecord(String ulogin, String upwd)
		{
			TinderLoadUserDB();
			var srec = "";
			foreach (var c in listTinderUserDB)
			{
				if (c.EndsWith("|" + ulogin + "|" + upwd))
				{
					srec = c;
					break;
				}
		...
		*/
		...
		private System.Collections.ObjectModel.Collection<VkNet.Model.Message> api_Messages_Get(out int _totalCount)
		{
			...
			if (iPersUserID >= 0 && SocialNetwork == 0 && fwbVKontakte != null)
			{
				/*
        ...
        private void addToHistory(long _iPersUserID, long _iContUserID, bool inboundMessage, string _date, string _time, string _text)
        {
            ...
            while (!success && attempts < maxAttempts)
            {
                try
                {
                    var listHistory = new List<string>(File.ReadAllLines(filePath));
                    ...
                    File.WriteAllLines(filePath, listHistory);
                    success = true;
                }
                catch (IOException)
                {
		...
		private void EditEQOutMessageParametersValues(String sMessage, String sMarker)
		{
			...
			FormEditEQMessagesDB formEditEQMessagesDB = new FormEditEQMessagesDB(this, false);
			formEditEQMessagesDB.sMsgHar = new String[iMsgHarCount, iMsgHarAttrCount + 1];
			for (int i = 0; i < iMsgHarCount; i++)
			{
				for (int j = 0; j < iMsgHarAttrCount; j++)
					formEditEQMessagesDB.sMsgHar[i, j] = sMsgHar[i, j];
				formEditEQMessagesDB.sMsgHar[i, iContHarAttrCount] = "";
			}
			formEditEQMessagesDB.iMsgHarCount = iMsgHarCount;
			formEditEQMessagesDB.iMsgHarAttrCount = iMsgHarAttrCount;
			formEditEQMessagesDB.Setup(iPersUserID.ToString() + " (" + userLogin + ", " + userPassword + ")", iPersUserID, sMsgNewRec);

			formEditEQMessagesDB.ShowDialog();
			SetEQOutMessageList(sCurrentEQInMessageRecord);
			if (formEditEQMessagesDB.bNeedSetMessage && formEditEQMessagesDB.SelectedMsgRecord.Length > 0)
			{
				if (!lstEQOutMessagesList.Contains(formEditEQMessagesDB.SelectedMsgRecord))
				{
					listBoxOutMsg.SelectedIndex = -1;
					lstEQOutMessagesList.Insert(0, formEditEQMessagesDB.SelectedMsgRecord);
					listBoxOutMsg.Items.Insert(0, "?% " + GetMessageTextWithMarker(formEditEQMessagesDB.SelectedMsgRecord.Substring(formEditEQMessagesDB.SelectedMsgRecord.IndexOf("|@!") + 3)));
					listBoxOutMsg.SelectedIndex = 0;
				}
				else
				{
					try
					{
						listBoxOutMsg.SelectedIndex = lstEQOutMessagesList.IndexOf(formEditEQMessagesDB.SelectedMsgRecord);
					}
					catch
					{
						listBoxOutMsg.SelectedIndex = -1;
				...
				if (bNotDelayed)
				{
					...
					if (moveLastContacterMessageToTop() && (!cmd_line.Equals("wall")) && (!cmd_line.Equals("friends")) && (!cmd_line.StartsWith("attach:")) && (!cmd_line.StartsWith("wallattach:")) && (!cmd_line.StartsWith("time_delay:")))
					{
						...
						if (lblEQOutHarValues[14].Text.IndexOf('%') > 0)
						{
							...
							try
							{
								checkMessageIgnor = Convert.ToInt32(value);

								if (checkMessageIgnor >= 0 && checkMessageIgnor <= 100)
								{
									Random rnd = new Random();
									int iZero = rnd.Next(0, 100);

									if (iZero > 100 - checkMessageIgnor)
										checkMessageIgnor = 0;
									else
										checkMessageIgnor = 100;
								}
			...
			foreach (var resp in responseInterface)
			{
				...
				else if (resp.STATUS == 200 && resp.DATA != null) // проверка есть ли новые сообщения у персонажа
				{
					foreach (var newmessage in resp.DATA)
					{
						...
						localContId = GetContactIdByParametrValue(6, newmessage.CONTACTER, localPersId);
						//реализация добавления контактера в БД по настройке алгоритма
						if (localContId == -1 && !adbrCurrent.bIgnoreMessagesFromNotContacter && newmessage.MESSAGES.Length > 0)
						{
							localContId = FindFreeIndex(lstContactsList);
							var userRec = $"{localContId}||";
							lstContactsList.Add(userRec);
			...
			for (var i = 0; contMessages.Count > 0; i++)
			{
				...
				if (adbrCurrent.SplitTextIntoSentences)
				{
					...
					while (contMessages[i].TEXT.Length > 0)
					{
						...
						msg.TYPE_STATUS = contMessages[i].TYPE_STATUS;
						msg.DATE_TIME = contMessages[i].DATE_TIME;
		...
		{
			_onChangePersoneStartServiceBool = _startservice;
	 ...
	 //               List<String> lstContHarCurrent = new List<String>();
		...
		private void tbEditContactsDB_Click(object sender, EventArgs e)
		{
			...
			if (formEditContactsDB.bNeedPersoneChange && formEditContactsDB.iContUserID >= 0)
			{
				StopService();
				iContUserID = formEditContactsDB.iContUserID;
				ContactsList_Load();
				LoadContactParamersValues();
				SetContactParametersValues();
				LoadContactParametersDescription();
			...
			else
			{
				...
				if (!INIT_PERSONE_DIALOG && formEditContactsDB.bInitDialogs)
					SelectNextReceivedMessage(false);
		...
		private void SetContactNamesAndPhoto()
		{
			labelCont1Name.Text = contNameName;
			labelCont1Family.Text = contNameFamily;
			contName = contNameName + " " + contNameFamily;
...

            {
				if (File.Exists(Path.Combine(sDataPath, "_contacts_" + getSocialNetworkPrefix() + Convert.ToString(iPersUserID) + ".txt")))
				{
					try
					{
						var contacters = File.ReadAllLines(Path.Combine(sDataPath, "_contacts_" + getSocialNetworkPrefix() + Convert.ToString(iPersUserID) + ".txt"));
						foreach (var c in contacters)
						{
				...
				if (photoContURL != "")
				{
					if (File.Exists(photoContURL))
					{
                        try
                        {
                            using (var image = Image.FromFile(photoContURL))
                            {
                                buttonEditContHarValues.BackgroundImage = (Image)image.Clone();
                            }
                        }
                        catch (Exception e)
                        {
		...
		private void tbEditPresonenDB_Click(object sender, EventArgs e)
		{
			...
			FormEditPersonenDB formEditPersonenDB = new FormEditPersonenDB(this);
			formEditPersonenDB.sContHar = new String[iPersHarCount, iPersHarAttrCount + 1];
			for (int i = 0; i < iPersHarCount; i++)
			{
				for (int j = 0; j < iPersHarAttrCount; j++)
					formEditPersonenDB.sContHar[i, j] = sPersHar[i, j];
				formEditPersonenDB.sContHar[i, iPersHarAttrCount] = "";
			}
			formEditPersonenDB.iContHarCount = iPersHarCount;
			...
			if (lstPersoneChange.Count > 0)
			{
				foreach (String str in lstPersoneChange)
					...
					{
						if (formEditPersonenDB.lvList.Items[i].SubItems[1].Text == str)
						{
							formEditPersonenDB.lvList.Items[i].Checked = true;
							break;
						}
					}
			}
			
			formEditPersonenDB.ShowDialog();
			...
			if (formEditPersonenDB.bNeedPersoneChange)
			{
				...
				SaveReceivedMessagesPull();
				SaveOutgoingMessagesPull();
				if (formEditPersonenDB.suSelSocialNetwork != SocialNetwork)
				{
					SocialNetwork = formEditPersonenDB.suSelSocialNetwork;
					OnSocialNetworkChanged();
				}
				onAfterPersonenListChanged();
				//LoadPersoneParametersValues();
		...
		private bool selectUserVKontakte()
		{
			...
			formEditPersonenDB.iContHarCount = iPersHarCount;
			formEditPersonenDB.iContHarAttrCount = iPersHarAttrCount;
			formEditPersonenDB.Setup("Init select user", -1);
			...
			return formEditPersonenDB.bNeedPersoneChange;
		}
		...
		private void OnSocialNetworkChanged()
		{
			if (SocialNetwork == 1)
			{
				...
				if (fwbVKontakte != null) fwbVKontakte.Close();
			}
			...
			{
				tbChangeCommunicationModeGroups.Enabled = true;
				tbChangeSocialNetwork.Image = global::Nilsa.Properties.Resources.social_tinder;

				iContactsGroupsMode = (lstPersoneChange.Count > 0 && SocialNetwork == 3) ? (ChangeModeWhenFree < 2 ? ChangeModeWhenFree : 1) : 0;
				tbChangeCommunicationMode.Image = Nilsa.Properties.Resources._mode_contacter;
				toolStripButtonChangeModeWhenFree.Checked = lstPersoneChange.Count > 0;// && SocialNetwork == 3;
				fwbVKontakte?.Close();
			}
		}
		...
		private void ChangeSocialNetwork(int newSocialNetwork)
		{
			...
			else if (SocialNetwork == 3)
			{
				...
				tbChangeCommunicationModeGroups.Enabled = false;
				tbChangeSocialNetwork.Image = global::Nilsa.Properties.Resources.social_tinder;
				//MessageBox.Show("Теперь используем Тиндер");
			}
			else
			{
		...
		private void tbSetSocialNetworkTinder_Click(object sender, EventArgs e)
		{
			//SocialNetwork = 0;
			//toolStripButtonSocialNetworkChange_Click(null, null);
			ChangeSocialNetwork(3);
		...
		private void tbEditGroupsDB_Click(object sender, EventArgs e)
		{
			...
			StopAnswerTimer();
			LoadGroupParametersDescription();
			FormEditGroupsDB formEditGroupsDB = new FormEditGroupsDB(this);
			formEditGroupsDB.sGroupHar = new String[iGroupHarCount, iGroupHarAttrCount + 1];
			for (int i = 0; i < iGroupHarCount; i++)
			{
				for (int j = 0; j < iGroupHarAttrCount; j++)
					formEditGroupsDB.sGroupHar[i, j] = sGroupHar[i, j];
				formEditGroupsDB.sGroupHar[i, iGroupHarAttrCount] = "";
			}
			formEditGroupsDB.iGroupHarCount = iGroupHarCount;
			formEditGroupsDB.iGroupHarAttrCount = iGroupHarAttrCount;
			formEditGroupsDB.Setup(iPersUserID.ToString() + " (" + userLogin + ", " + userPassword + ")", iPersUserID, sGroupAdditinalUsers, INIT_PERSONE_DIALOG);

			try
			{
				formEditGroupsDB.ShowDialog();
				Application.DoEvents();
			}
			catch
			{
			...
			if (formEditGroupsDB.bNeedPersoneChange && formEditGroupsDB.iSelGroupID >= 0)
			{
				StopService();
				iContUserID = -formEditGroupsDB.iSelGroupID;
				//iContUserID = -1;
			...
			else
			{
				...
				//comboBoxCompareVectorLevel_SelectedIndexChanged(sender, e);
				comboBoxCompareLexicalLevel_SelectedIndexChanged(sender, e);
				if (!INIT_PERSONE_DIALOG && formEditGroupsDB.bInitDialogs)
					SelectNextReceivedMessage(false);
		...
		private OutgoingMessage OutgoingMsgParser(string outMsg)
		{
			...
			outMsg = outMsg.Substring(outMsg.IndexOf("|") + 1);
			//берем сообщение
			//outMessages.Message = outMsg;
            //if (outMessages.Message.Contains("MESSAGE:")) outMessages.Message = ChangeBrToSlashN(outMessages.Message);
		...
		private string ChangeBrToSlashN(string changingText)
		{
            int messageIndex = changingText.IndexOf("MESSAGE:"); // Находим индекс начала атрибута MESSAGE:
            if (messageIndex != -1)
            {
                int nextAttributeIndex = changingText.IndexOf("<br>", messageIndex + 8);
                if (nextAttributeIndex != -1)
                {
                    string messagePart = changingText.Substring(messageIndex + 8, nextAttributeIndex - (messageIndex + 8));

                    // Заменяем <br> на \n в тексте сообщения
                    string replacedMessagePart = Regex.Replace(messagePart, "<br>", "\n");

                    // Обновляем полный текст outMsg с заменой
                    changingText = changingText.Substring(0, messageIndex + 8) + replacedMessagePart + changingText.Substring(nextAttributeIndex);
                }
            }
            return changingText;
        }
		...
		private void backgroundWorkerReadNewMessagesTimer_DoWork(object sender, DoWorkEventArgs e)
		{
			if (lstReceivedMessages.Count == 0)
			{
                ...
                {
					if (lstOutgoingMessages[i].Contains("_SELENIUM_NEW_MESSAGES") && lstOutgoingMessages[i].Contains("*#|0|0|The System|"))
					{
						needAdd = false;
						break;
                    }
